...
# Set the page config first (only once)
...
def plot_spread(spread, pair, B):
    ...
    quantiles = {}
    for label, (lower_q, upper_q) in q_levels.items():
        quantiles[label] = {
            'lower': spread.quantile(lower_q),
            'upper': spread.quantile(upper_q)
        }
...
with col1:
    analysis_start_date = st.date_input("Start Date", value=one_year_one_day_ago, key="analysis_start_date")
with col2:
    analysis_end_date = st.date_input("End Date", value=today, key="analysis_end_date")
...
classification_dict = {row['Ticker']: (row['Classification'], row['Leverage']) for _, row in classification_df.iterrows()}

if st.button("Run Cointegration Analysis"):
    st.write("Running analysis, please wait...")
    tickers = list(classification_dict.keys())
    price_data = fetch_price_data(tickers, str(analysis_start_date), str(analysis_end_date))
    ...
    if price_data.empty:
        ...
        for _, row in optimized_df.iterrows():
            s1, s2 = row['Stock1'], row['Stock2']
            if s1 not in classification_dict or s2 not in classification_dict:
                continue
            underlying1, lev1 = classification_dict[s1]
            underlying2, lev2 = classification_dict[s2]
...
if 'final_pairs' not in st.session_state or st.session_state['final_pairs'].empty:
    ...
    st.write("Choose a different timeframe for viewing the spreads.")
    col3, col4 = st.columns(2)
    with col3:
        view_start_date = st.date_input("Start Date", value=six_months_one_day_ago, key="view_start_date")
    with col4:
        view_end_date = st.date_input("End Date", value=today, key="view_end_date")
    ...
    with col5:
        BACKTEST_START_DATE = st.date_input("Start Date", value=one_year_one_day_ago, key="backtest_start_date")
    with col6:
        BACKTEST_END_DATE = st.date_input("End Date", value=today, key="backtest_end_date")
