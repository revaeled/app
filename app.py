...
names = ['Derick Ip']
usernames = ['derick']
passwords = ['d3r1ck!']  # Plaintext password
...
if authentication_status == False:
    ...
    # User is authenticated, show the rest of the app
    ...
    # UI Layout
    # --------------------------------------------
    st.markdown("<div class='title'>Cointegrated ETFs Trading Strategy App</div>", unsafe_allow_html=True)
    ...
    st.markdown("<div class='section-header'>Step 1: Cointegration Analysis (Part A & B)</div>", unsafe_allow_html=True)
    col1, col2 = st.columns(2)
    with col1:
        start_date = st.date_input("Analysis Start Date", value=pd.to_datetime("2023-12-05"))
    with col2:
        end_date = st.date_input("Analysis End Date", value=pd.to_datetime("2024-12-10"))
    ...
    if 'final_pairs' not in st.session_state or st.session_state['final_pairs'].empty:
        ...
        st.markdown("<div class='section-header'>Step 2: Visualize Spread (Part C)</div>", unsafe_allow_html=True)
        st.write("Here you can choose a different timeframe for viewing the spreads.")
        col3, col4 = st.columns(2)
        with col3:
            view_start_date = st.date_input("Part C View Start Date", value=pd.to_datetime("2024-06-05"))
        with col4:
            view_end_date = st.date_input("Part C View End Date", value=pd.to_datetime("2024-12-10"))

        # Choose view mode: single pair or all pairs
        c_view_mode = st.radio("View Mode", ["All Pairs", "Single Pair"])

        if c_view_mode == "Single Pair":
            selected_pair_c = st.selectbox("Select a pair to plot (Part C)", final_pairs['Pair'].unique())
        else:
            selected_pair_c = None

        if st.button("View Part C Plots"):
            ...
            if c_price_data.empty:
                st.error("No data for the selected timeframe in Part C.")
            else:
        ...
        st.markdown("<div class='section-header'>Step 3: Backtest Strategy (Part D)</div>", unsafe_allow_html=True)
        st.write("Select a timeframe for the backtest.")

        col5, col6 = st.columns(2)
        with col5:
            BACKTEST_START_DATE = st.date_input("Backtest Start Date", value=pd.to_datetime("2023-12-06"))
        with col6:
            BACKTEST_END_DATE = st.date_input("Backtest End Date", value=pd.to_datetime("2024-12-10"))
        ...
        # Choose view mode for Part D
        d_view_mode = st.radio("View Mode for Backtest Results", ["All Pairs", "Single Pair"])

        if d_view_mode == "Single Pair":
            selected_pair_d = st.selectbox("Select a pair to see backtest plot (Part D)", final_pairs['Pair'].unique())
        else:
